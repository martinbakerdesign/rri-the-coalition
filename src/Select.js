...
class Select {
  constructor(el, idPrefix = defaultIdPrefix, onChangeCallback = null) {
    ...
    this.id = (el.id && el.id.length)
      ? el.id
      : this.idPrefix.slice(0,-2);
  ...
  onToggleKeyDown = (event) => {
    ...
    switch (action) {
        ...
        case SelectActions.PageDown:
            ...
            return this.onOptionFocus(
              this.getUpdatedIndex(action)
            );
  ...
  setValue = (newValue) => {
    const option = this.options.find(option => option.dataset.value.toString() === newValue.toString());
